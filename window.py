...
class Window(tk.Tk):
    ...
    def frame_tables_lists(self):
        ...
        frame_tables_lists = tk.Frame(
            self,
            height=670,
            width=300,
            bg='#C0C0C0'
        )
        frame_tables_lists.place(
            relx=0,
            rely=0.05,
            relwidth=0.3,
            relheight=0.95
        ...
        get_btn = tk.Button(
            master=frame_tables_lists,
            text='Получательная кнопочка',
            bg='#FF69B4',
            fg='#00FF00',
            activebackground='#00BFFF',
            font=self.FONT,
            activeforeground='#FF0000',
            command=lambda: self.frame_current_table([])
        )
        get_btn.place(relx=0.3, rely=0.6)
        db_btn = tk.Button(
            master=frame_tables_lists, text='Подключение',
            bg='#00FF00',
            fg='#C71585',
            activebackground='#FFFF00',
            font=self.FONT,
            activeforeground='#FF0000',
            command=self.__db_con
        )
        db_btn.place(relx=0.5, rely=0.4)
        db_btn = tk.Button(
            master=frame_tables_lists,
            text='Выбирательная кнопочка',
            bg='#800080',
            fg='#F0FFFF',
            activebackground='#1E90FF',
            font=self.FONT,
            activeforeground='#800000',
            command=lambda: [self.choose_msdb(), self.frame_tables_lists()]
        )
    ...
    def frame_midle_btns(self):
        ...
        editor_btn = tk.Button(
            master=frame_midle_btns,
            text='Выполнить',
            bg='#778899',
            font=self.FONT,
            command=lambda: [self._db.execute_query(
                self.query.get('1.0', tk.END).strip()),
                self.select_query()])
        drop_btn = tk.Button(
            master=frame_midle_btns,
            text='Очистить',

...
class Window(tk.Tk):
    ...
    def frame_midle_btns(self):
        ...
        editor_btn = tk.Button(master=frame_midle_btns, text='Выполнить', bg='#778899', font=self.FONT,
                               command=lambda: [DbWorker().execute_query(self.query.get('1.0', tk.END).strip()), self.frame_current_table()])
    ...
    def frame_current_table(self):
        ...
        lst = DbWorker().show_table(self.tables_lst.get())
        table = ttk.Treeview(master=frame_current_table, show='headings')
        heads = DbWorker().get_column_names(self.tables_lst.get())
        table['column'] = heads
        for header in heads:
